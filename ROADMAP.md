...
>
> **Dernière mise à jour** : 2026-06-19 · **Version courante** : `2.10.0`
...
| 13 | **Export PDF Premium** : `src/lib/pdf-export.ts` (jsPDF chargé via CDN, 0 ajout au bundle, génération 100 % navigateur), bouton « 📄 Export PDF » par configuration sur `/account/configurations` — **réservé aux premium** (gratuit → 🔒 redirige `/pricing`). Fiche PDF de marque (en-tête vert, tableau raquette/cordages/tensions/RCS/compat, notes, pied de page). Rendu vérifié visuellement. | Option A / #3.2 | #43 | `696ff91` | **2.9.0** |
| 14 | **Import article blog EN** : article « What Racquets & Strings Do Fonseca, Mensík, Cobolli & Jódar Use? (2026 Gear Breakdown) » importé dans `public/en/blog/next-gen-tennis-racquets-fonseca-mensik-cobolli-jodar.html`. Mise aux normes EN : canonical/og/breadcrumb/Schema → `/en/blog/<slug>.html`, hreflang `en`+`x-default`, **analytics** (`/js/analytics.js` + gtag `G-YSSLHJ5WYD`), liens « related » recâblés vers articles réels (FR + article EN existant). Carte ajoutée sur `public/en/blog/index.html`. **Sitemap** étendu (`/en/blog/` + 2 articles EN, auparavant absents). | Contenu/SEO | #44 | `f6ff965` | **2.9.1** |
| 15 | **Version EN accessible depuis l'app** : le site anglais statique (`public/en/*.html`, dont le blog) était **orphelin** — le toggle FR/EN ne faisait que traduire l'UI en place, aucun lien n'y menait. ➜ `src/lib/i18n/route-map.ts` (mapping FR app ↔ EN statique, fallback home si pas d'équivalent, query/hash ignorés), `LanguageToggle` transformé en **vraie navigation** vers la page EN équivalente (`/`→`/en/`, `/configurator`→`/en/configurator.html`, `/pricing`→`/en/premium.html`, `/blog/`→`/en/blog/`…), lien explicite **« 🇬🇧 English »** ajouté au footer. Bonus : correction de clés React dupliquées dans le footer. Mapping testé 12/12, prod `/en/` & `/en/blog/` = HTTP 200. | UX/i18n | #45 | `8ac27db` | **2.9.2** |
| 16 | **RCS avancé v1 (Premium)** : nouvelle analyse multi-facteurs gardant le RCS simple comme socle. `src/lib/advanced-rcs.ts` (fonction pure `calculateAdvancedRcs`, **0 modification des formules existantes**) calcule **5 sous-scores 0-100** (Contrôle, Puissance, Confort, Spin, Durabilité) + un score global pondéré (confort prioritaire = santé du bras), des **recommandations personnalisées** et des **avertissements bras** (alertes pour profils sensibles + setups rigides). Intégré au configurateur (`src/app/configurator/page.tsx`) avec **gating Premium** : RCS simple reste gratuit, bloc « 🔬 Analyse RCS Avancée » flouté + CTA `/pricing` pour les comptes gratuits. Concrétise la promesse « Analyse RCS avancée » + « Recommandations personnalisées » de la page Premium. Logique validée par esbuild sur 4 cas réels (poly rigide + bras sensible, boyau souple, débutant + poly, hybride). | Monétisation / #3.2 | _en cours_ | _en cours_ | **2.10.0** |
